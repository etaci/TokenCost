...
use fusebox_ledger::{BreakerEvent, BreakerTransitionKind, SharedLedger, SpendQuery};
...
impl PolicyEngine {
    ...
    fn breaker_for(
        ...
        self.breakers.entry(tenant.clone()).or_insert_with(|| {
            parking_lot::Mutex::new(Breaker::new(cfg.breaker_cooldown, cfg.halfopen_trials))
        });
        self.breakers
            .get(tenant)
    ...
    fn detector_for(
        ...
        self.detectors.entry(tenant.clone()).or_insert_with(|| {
            EwmaDetector::new(cfg.anomaly_alpha, cfg.anomaly_sigma, cfg.anomaly_warmup)
        });
    ...
    pub async fn spend_for(&self, tenant: &TenantId, window: BudgetWindow) -> Result<CostUsd> {
        let totals = self
            .ledger
            .totals(&SpendQuery::for_tenant_since(
                tenant.clone(),
    ...
    pub async fn record_outcome(
        ...
        if !succeeded {
            ...
            if let Some(t) = &trans {
                self.audit_transition(tenant, t, Some("upstream_failure"))
                    .await;
            }
            return Ok(trans);
        }
        ...
        if let Some(t) = &success_transition {
            self.audit_transition(tenant, t, Some("halfopen_recovered"))
                .await;
        }
...
mod tests {
    ...
    async fn anomaly_detector_trips_on_spike() {
        ...
        for _ in 0..20 {
            let _ = engine
                .record_outcome(&tenant, CostUsd(0.01), true)
                .await
                .unwrap();
        }
        // Sudden 100x spike.
        let t = engine
            .record_outcome(&tenant, CostUsd(1.0), true)
            .await
            .unwrap();

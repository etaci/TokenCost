...
pub trait LedgerStore: std::fmt::Debug + Send + Sync + 'static {
    ...
    /// Store a budget request. Used by the approval workflow to persist
    /// requests so they survive restarts.
    async fn store_budget_request(&self, _request: &BudgetRequestRecord) -> Result<()> {
        Ok(())
    }

    /// Update an existing budget request's status and decision fields.
    async fn update_budget_request_status(
        ...
        _decision_note: Option<&str>,
    ) -> Result<()> {
        Ok(())
    }

    /// List budget requests matching the query.
    async fn list_budget_requests(
        &self,
        _query: &BudgetRequestQuery,
    ) -> Result<Vec<BudgetRequestRecord>> {
        Ok(Vec::new())
    }

    /// Get a single budget request by ID.
    async fn get_budget_request(&self, _id: &str) -> Result<Option<BudgetRequestRecord>> {
        Ok(None)
    }
}
...
/// Persisted budget request record.
#[derive(Debug, Clone)]
...
/// Query for filtering budget requests.
#[derive(Debug, Clone)]
pub struct BudgetRequestQuery {

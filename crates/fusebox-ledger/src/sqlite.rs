...
impl LedgerStore for SqliteLedger {
    ...
    async fn list(&self, query: &SpendQuery) -> Result<Vec<SpendEvent>> {
        ...
        for row in rows {
            ...
            let event = SpendEvent {
                ...
                status: SpendStatus::from_str(&status_str).unwrap_or(SpendStatus::Completed),
    ...
    async fn store_budget_request(&self, request: &BudgetRequestRecord) -> Result<()> {
        ...
        .bind(request.ttl_seconds.map(|t| t as i64))
        .execute(&self.pool)
        .await
        .map_err(|e| FuseboxError::Storage(e.to_string()))?;
        Ok(())
    ...
    async fn list_budget_requests(
        ...
        let mut out = Vec::with_capacity(rows.len());
        for row in rows {
            let requested_at_str: String = row
                .try_get("requested_at")
                .map_err(|e| FuseboxError::Storage(e.to_string()))?;
            let decided_at_str: Option<String> = row.try_get("decided_at").ok();

            out.push(BudgetRequestRecord {
                id: row
                    .try_get("id")
                    .map_err(|e| FuseboxError::Storage(e.to_string()))?,
                tenant_id: row
                ...
                window: row
                    .try_get("window")
                    .map_err(|e| FuseboxError::Storage(e.to_string()))?,
                requested_limit_usd: row
                    .try_get("requested_limit_usd")
                    .map_err(|e| FuseboxError::Storage(e.to_string()))?,
                ...
                ttl_seconds: row.try_get::<i64, _>("ttl_seconds").ok().map(|t| t as u64),
            });
        }
        Ok(out)
    }

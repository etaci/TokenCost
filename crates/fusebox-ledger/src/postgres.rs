...
use crate::store::{
    BreakerEventQuery, BudgetRequestQuery, BudgetRequestRecord, LedgerStore, SpendQuery,
    SpendTotals,
};
...
impl LedgerStore for PgLedger {
    ...
    async fn store_budget_request(&self, request: &BudgetRequestRecord) -> Result<()> {
        let id = Uuid::parse_str(&request.id).unwrap_or_else(|_| Uuid::new_v4());
        sqlx::query(
            r#"
            INSERT INTO budget_requests (
                id, tenant_id, window, requested_limit_usd, reason,
                requested_at, status, decided_at, decided_by, decision_note, ttl_seconds
        ...
        .bind(&request.window)
        .bind(request.requested_limit_usd)
        ...
        .bind(request.ttl_seconds.map(|t| t as i32))
        .execute(&self.pool)
        .await
        .map_err(|e| FuseboxError::Storage(e.to_string()))?;
        Ok(())
    ...
    async fn update_budget_request_status(
        &self,
        id: &str,
        status: &str,
        ...
        let id = Uuid::parse_str(id).map_err(|e| FuseboxError::Storage(e.to_string()))?;
        sqlx::query(
            r#"
    ...
    async fn list_budget_requests(
        &self,
        query: &BudgetRequestQuery,
    ) -> Result<Vec<BudgetRequestRecord>> {
        let limit = query.limit.unwrap_or(100).min(1000) as i32;

        let rows = match (&query.tenant, &query.status) {
            (Some(tenant), Some(status)) => sqlx::query(
                r#"
                SELECT id, tenant_id, window, requested_limit_usd, reason,
                       requested_at, status, decided_at, decided_by, decision_note, ttl_seconds
        ...
        Ok(rows.into_iter().map(row_to_budget_request).collect())
    }

    async fn get_budget_request(&self, id: &str) -> Result<Option<BudgetRequestRecord>> {
        let id = Uuid::parse_str(id).map_err(|e| FuseboxError::Storage(e.to_string()))?;
        let row = sqlx::query(
            r#"
            SELECT id, tenant_id, window, requested_limit_usd, reason,
        ...
        Ok(row.map(row_to_budget_request))
    }
}

fn row_to_budget_request(row: PgRow) -> BudgetRequestRecord {
    let id: Uuid = row.try_get("id").unwrap_or_else(|_| Uuid::new_v4());
    BudgetRequestRecord {
        id: id.to_string(),
        tenant_id: row.try_get("tenant_id").unwrap_or_default(),
        window: row.try_get("window").unwrap_or_default(),
        requested_limit_usd: row.try_get::<f64, _>("requested_limit_usd").unwrap_or(0.0),
        ...
        ttl_seconds: row.try_get::<i32, _>("ttl_seconds").ok().map(|t| t as u64),
    }
}
...
fn row_to_event(row: PgRow) -> SpendEvent {
    ...
    SpendEvent {
        ...
        request_id,
        status: SpendStatus::from_str(&status).unwrap_or(SpendStatus::Completed),
        metadata,
    }
}
...
fn row_to_breaker_event(row: PgRow) -> BreakerEvent {
    ...
    BreakerEvent {
        ...
        transition: BreakerTransitionKind::from_str(&transition)
            .unwrap_or(BreakerTransitionKind::Close),
        reason,
    }
}
...
mod tests {
    ...
    fn splits_sql_around_dollar_blocks() {
        let sql =
            "CREATE TABLE a (id INT); DO $$ BEGIN PERFORM 1; END $$; CREATE INDEX i ON a(id);";

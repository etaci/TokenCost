...
async fn sync(args: SyncArgs) -> Result<()> {
    ...
    // we list above.
    let raw: serde_json::Value = serde_json::from_str(&body).context("parse LiteLLM JSON")?;
    let obj = raw
        .as_object()
    ...
    if !args.dir.exists() {
        std::fs::create_dir_all(&args.dir)
            .with_context(|| format!("create pricing dir {}", args.dir.display()))?;
    }
    ...
    for (raw_name, value) in obj.iter() {
        ...
        let Some((_, file_stem, _)) = PROVIDERS
            .iter()
            .find(|(p, _, _)| matches_provider(provider, p))
        else {
            continue;
        };
    ...
    for (_, file_stem, provider_enum) in PROVIDERS {
        ...
        if normalise(&existing) == normalise(&body) {
            println!("  {}.yaml: {} models — unchanged", file_stem, models.len());
            continue;
        }
        if args.check {
            println!(
...
fn dump_yaml(
    provider_enum: &str,
    last_updated: &str,
    models: &BTreeMap<String, ModelOut>,
) -> String {

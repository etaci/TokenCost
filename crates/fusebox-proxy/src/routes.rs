...
use fusebox_core::{
    BudgetWindow, CostUsd, Decision, DenyReason, PricingTable, Provider, TenantId, TokenUsage,
};
...
pub fn router(state: AppState) -> Router {
    Router::new()
        ...
        .route(
            "/v1/budget/requests/:id/approve",
            post(budget_request_approve),
        )
        .route(
            "/v1/budget/requests/:id/reject",
            post(budget_request_reject),
        )
        .route("/v1/mcp/budget", get(mcp_get_budget))
        .route("/v1/mcp/spend", get(mcp_get_spend))
        .route("/v1/mcp/breaker", get(mcp_get_breaker))
        .route(
            "/v1/mcp/budget/request_increase",
            post(mcp_request_increase),
        )
...
async fn events_list(
    ...
    let seconds = q.seconds.unwrap_or(3600).clamp(60, 7 * 24 * 3600);
...
async fn budget_requests_list(
    ...
    json_response(
        StatusCode::OK,
        &json!({ "requests": items, "count": count }),
    )
}
...
async fn proxy_request_inner(
    ...
    record_completed(&state, &id, &parsed, kind.provider(), real_usage, real_cost).await;
    let _ = state
        .policy
...
fn build_response(
    status: StatusCode,
    src_headers: &reqwest::header::HeaderMap,
    body: Body,
) -> Response<Body> {

...
pub async fn reconcile(mut rx: mpsc::UnboundedReceiver<Bytes>, flavor: StreamFlavor) -> TokenUsage {
...
fn fold_anthropic(v: &Value, event_type: &str, usage: &mut TokenUsage) {
    ...
    let inferred_type = v.get("type").and_then(Value::as_str).unwrap_or(event_type);

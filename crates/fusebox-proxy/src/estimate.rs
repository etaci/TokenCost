...
pub fn parse_openai_chat(body: &[u8], pricing: &PricingTable) -> Result<ParsedRequest, String> {
    let parsed: ChatCompletionsBody =
        serde_json::from_slice(body).map_err(|e| format!("invalid OpenAI request body: {e}"))?;
...
pub fn parse_anthropic_messages(
    ...
    let parsed: AnthropicMessagesBody =
        serde_json::from_slice(body).map_err(|e| format!("invalid Anthropic request body: {e}"))?;
    if parsed.model.is_empty() {
        return Err("`model` field is required".to_string());
    }
    let model = ModelId::new(parsed.model.clone());
...
pub fn parse_google_generate_content(
    ...
    let parsed: GoogleGenerateBody =
        serde_json::from_slice(body).map_err(|e| format!("invalid Google request body: {e}"))?;
    if parsed.model.is_empty() {
